...
forwardFeatureAddition <- function(a,r) {
  ...
  repeat{
    ...
    if(pPrev-p>0.03){
      #If gets worse, get rid of last feature added
      features <- features[-(ncol(aCluster))]
  ...
  return(list(p=pPrev,aCluster=aCluster,rFeatures=features,cl=cl))
...
#########################################################################
getGroups <- function(aIni,r,fAddOut,print=FALSE) {
  pPrev <- fAddOut$p
  aCluster <- fAddOut$aCluster
  cl <- fAddOut$cl
  relevantFeatures <- fAddOut$rFeatures
  if (print==TRUE) {
    print("r: ")
    print(r)
    print("Final p-value")
...
#         # relevantFeatures[k]=l vol dir feature l, columna l+1 de a
#       }
#     }
#   }
#     
  ...
  for(i in 1:nrow(a)){
    if(cl$cluster[[i]] == 1){
      ...
      Group1 <- cbind(Group1,i)
    }
    else{
      # Group2<-cbind(Group2,a[i,1])
      Group2 <- cbind(Group2,i)
    }
  }
  return(list(g1=Group1,g2=Group2))
}
#########################################################################
...
computeWithSeed <- function(a,seed,threshold) {
  ...
  return(list(accuracy=accuracy,result=f,groups=groups))
}
#########################################################################
...
#   LOOP THROUGH ALL THE SEEDS   ########################################
#########################################################################
fasterSeedLoop <- function(a,minSeed,maxSeed,threshold) {
  bestAcc <- 0
  for (seed in minSeed:maxSeed) {
    print(paste("Computing seed",seed))
    temp <- computeWithSeed(a,seed,threshold)
    if(temp$accuracy>bestAcc) {
      bestAcc <- temp$accuracy
      bestSeed <- seed
      bestFit <- temp
    }
  }
  return(list(fit=bestFit,seed=bestSeed,accuracy=bestAcc))
}
#########################################################################

#   GET LABELS OF THE FEATURES   ########################################
#########################################################################
getModelFeatures <- function(relevantFeatures,inputFeatures) {
  featureList <- inputFeatures$feature
  featureIndex <- inputFeatures$feature_number
  modelFeatures <- data.frame(index=seq(1,length(relevantFeatures)))
  modelFeatures$names <- featureList[relevantFeatures]
  modelFeatures$realIndex <- featureIndex[relevantFeatures]
  return(modelFeatures)
}
#########################################################################


#   CORRELATION BETWEEN FEATURES AND PISA SCORES   ######################
#########################################################################
featurePisaCorr <- function(mat) {
  result <- data.frame(index=seq(1,ncol(mat)-1))
  corrs <- numeric(ncol(mat)-1)
  for (i in 2:ncol(mat)) {
    corrs[i-1] <- cor(mat[,1],mat[,i])
  }
  result$cor <- corrs
  return(result);
}
...
#   CORRELATION BETWEEN FEATURES AND PISA SCORES FOR GOOD COUNTRIES   ###
#########################################################################
featurePisaCorr_Good <- function(mat,goodCountries) {
  result <- data.frame(index=seq(1,ncol(mat)-1))
  corrs <- numeric(ncol(mat)-1)
  for (i in 2:ncol(mat)) {
    corrs[i-1] <- cor(mat[goodCountries,1],mat[goodCountries,i])
  }
  result$cor <- corrs
  return(result);
}
...
#   CORRELATION BETWEEN FEATURES AND PISA SCORES   ######################
#########################################################################
featurePisaCorr_Bad <- function(mat,badCountries) {
  result <- data.frame(index=seq(1,ncol(mat)-1))
  corrs <- numeric(ncol(mat)-1)
  for (i in 2:ncol(mat)) {
    corrs[i-1] <- cor(mat[badCountries,1],mat[badCountries,i])
  }
  result$cor <- corrs
  return(result);
}
...
getCountryNames <- function(namelist,group) {
  return(namelist[group])
}